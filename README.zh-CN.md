...
- **聚合**——同时关注多个频道，把每条命中的 AI 帖文单独发进你的私聊。
...
**一条消息只判一次。** 刷新按**源频道**调度而非按订阅：频道上所有活跃模板的问题合并（去重 + 缓存），因此无论多少订阅者，每条新帖只花**一次 Jev 调用**。命中后按目的地对比过去 24 小时已成功发送的正文；近似重复的帖文不再发往同一目的地（[方法与边界](docs/DEDUPLICATION.md)）。
